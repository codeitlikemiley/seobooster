    ...
    >
        <template slot="headers" slot-scope="props">
            ...
            <th class="text-xs-center">
                ...
                <v-btn v-else flat icon color="red darken-4" @click.native="deleteSelected()">
        ...
        <template slot="items" slot-scope="props">
            ...
            >
              ...
              <div slot="input" class="mt-3 title">Update Username</div>
              <v-text-field
                slot="input"
                label="Edit Username"
                v-model="tmp"
            ...
            <td class="text-xs-center">
                <v-chip :class="{ 'green': isActive(props.item),'amber': !isActive(props.item) }" text-color="white">
                <span>{{ `${props.item.active ? 'Active' : 'Inactive'}` }}</span>
                </v-chip>
            </td>
            <td class="text-xs-left">{{ props.item.activated_at }}</td>
            <td class="text-xs-left">{{ props.item.expired_at }}</td>
            <th class="text-xs-center">{{  props.item.post_count }}</th>
            <td class="text-xs-center">
                <!-- //! View The User Profile Page -->
                <v-btn flat icon color="cyan" @click.native="">
                ...
                <v-btn v-if="tokenExpired(props.item)" flat icon color="indigo darken-2" @click.native="">
...
export default {
    ...
    data: () => ({
        ...
        headers: [
            ...
            { text: 'Token Creation Date', value: 'activated_at', align: 'left', sortable: true },
            { text: 'Token Expiration Date', value: 'expired_at', align: 'left', sortable: true },
