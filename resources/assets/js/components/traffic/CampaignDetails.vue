<template>
  <v-container>
          ...
          <v-layout row>
            <v-flex xs12 md8 offset-md2>
                  <v-radio-group v-model="traffic_type" :mandatory="true" row>
                    <v-radio color="primary" label="Via Search Engine" value="search_engine"></v-radio>
                    <v-radio color="info" label="Via Direct Traffic" value="direct_traffic"></v-radio>
                  </v-radio-group>
            </v-flex>
          </v-layout>
          <v-layout row>
          ...
          <v-layout row v-if="traffic_type === 'search_engine'">
            <v-flex xs12 md8 offset-md2>
                  <v-radio-group v-model="search_engine_type" :mandatory="true" row>
                    <v-radio color="blue darken-2" label="Google Search Engine"  value="google" prepend-icon="fa-google"></v-radio>
                    <v-radio color="red darken-4" label="Youtube Search Engine"  value="youtube" prepend-icon="fa-youtube"></v-radio>
                  </v-radio-group>
            </v-flex>
          </v-layout>
          <v-layout row v-if="traffic_type === 'search_engine'">
            <v-flex xs12 md8 offset-md2 v-if="search_engine_type === 'google'">
              <v-select
          ...
          <v-layout row v-if="traffic_type === 'search_engine'">
            <v-flex xs12 md8 offset-md2 v-if="search_engine_type === 'youtube'">
              <v-select
                v-model="search_engine"
          ...
          <v-layout row wrap>
            <v-flex xs12 md8 offset-md2>
                  <v-switch :label="`Geo Location Tracking: ${geo_tracking === true ? 'ON' : 'OFF'}`" v-model="geo_tracking"></v-switch>
            </v-flex>
          </v-layout>
          <v-layout row wrap v-if="geo_tracking">
           ...
           <v-layout row wrap>
            <v-flex xs12 md8 offset-md2>
                  <v-switch :label="`Internal Browsing: ${internal_browsing === true ? 'ON' : 'OFF'}`" v-model="internal_browsing"></v-switch>
            </v-flex>
          </v-layout>
          <v-layout row wrap v-if="internal_browsing">
            <v-flex xs12 md8 offset-md2>
                  <v-switch :label="`Random Browsing: ${random_browsing === true ? 'ON' : 'OFF'}`" v-model="random_browsing"></v-switch>
            </v-flex>
          </v-layout>
          <v-layout row wrap v-if="random_browsing === false">
            <v-flex xs12 md8 offset-md2>
                 <v-select
                        v-model="sites"
                        label="Add Site Urls"
          ...
          <v-layout row wrap>
            <v-flex xs12 md8 offset-md2>
                  <v-switch :label="`Bounce Back: ${bounce_back === true ? 'ON' : 'OFF'}`" v-model="bounce_back"></v-switch>
            </v-flex>
          </v-layout>
          <v-layout row wrap>
            <v-flex xs12 md8 offset-md2>
                  <v-switch :label="`Play Video Embeds: ${play_video_embeds === true ? 'ON' : 'OFF'}`" v-model="play_video_embeds"></v-switch>
            </v-flex>
          </v-layout>
          <v-layout row wrap v-if="play_video_embeds">
            <v-flex xs12 md8 offset-md2>
                  <v-text-field
          ...
          <v-layout row wrap>
            <v-flex xs12 md8 offset-md2>
                  <v-switch :label="`Social Sharing: ${social_sharing === true ? 'ON' : 'OFF'}`" v-model="social_sharing"></v-switch>
            </v-flex>
          </v-layout>
          <v-layout row wrap v-if="social_sharing">
            <v-flex d-flex xs12 md8 offset-md2>
                  <v-layout row wrap>
                    <v-flex xs6 d-flex pa-1>
                      <v-text-field
          ...
          <v-layout row wrap>
              <v-flex xs12 md8 offset-md2>
                  <v-btn block color="accent" class="white--text">Update Campaign <v-icon right>send</v-icon></v-btn>
              </v-flex>
          </v-layout>
        </v-container>
</template>
