...
<main-layout  :style="{ paddingTop: `100px`, backgroundColor: `white` }">
    <v-container  fluid>
        <action-panel :lifetime="lifetime" :monthly="monthly" :project-count="socialCount"></action-panel>
        <v-layout row wrap>
            <v-flex xs6 md4>
                <v-btn color="accent" dark @click.native="AddCampaign()">Add New Campaigns<v-icon right>fa-plus</v-icon></v-btn>
            </v-flex>
            <v-flex xs6 md4 offset-md4 text-xs-right>
                <v-btn color="lime lighten-2" dark>Total Credits<v-icon right>fa-money</v-icon></v-btn>
            </v-flex>
        </v-layout>
